...
class IsSellerOrAdmin(BasePermission):
    ...
    def has_permission(self, request, view):
        if request.user.is_authenticated:
            try:
                return SellerProfile.objects.get(id=request.user.id, is_seller=True)
            except SellerProfile.DoesNotExist:
                raise PermissionDenied({"Permission denied": "Account does not have permission to create product."})
        raise PermissionDenied({"Permission denied": "Only admin or seller users can create product."})
